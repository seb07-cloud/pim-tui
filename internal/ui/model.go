...
type clientReadyMsg struct {
	client     *azure.Client
	authMethod string // "cached browser auth", "Azure CLI", or "browser auth"
}
...
func initClientCmd() tea.Cmd {
	return func() tea.Msg {
		...
		// First, try cached browser authentication (silent auth)
		client, err := azure.NewClientWithCachedAuth(ctx)
		if err == nil && client != nil {
			// Verify cached auth works
			_, err = client.GetCurrentUser(ctx)
			if err == nil {
				return clientReadyMsg{client, "cached browser auth"}
			}
		}
		...
		if err != nil {
			// Check if this is an auth-related error that can be resolved with browser login
			if isAuthError(err) {
		...
		return clientReadyMsg{client, "Azure CLI"}
	}
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		// Update tree view dimensions if active
		if m.state == StateTreeView {
			m.treeView.SetSize(msg.Width-4, msg.Height-10)
		}
		return m, nil
	...
	case clientReadyMsg:
		...
		m.log(LogInfo, "Authenticated via %s", msg.authMethod)
		m.log(LogDebug, "Auth cache: %s", azure.GetAuthCachePath())
	...
	case authCompleteMsg:
		...
		m.loading = true
		m.loadingMessage = "Loading tenant info..."
		m.log(LogInfo, "Authenticated via browser (cached for future)")
		m.log(LogDebug, "Auth cache: %s", azure.GetAuthCachePath())
		return m, loadTenantCmd(m.client)
	...
	case animationTickMsg:
		// Route animation ticks to tree view when animating
		if m.state == StateTreeView && m.treeView.IsAnimating() {
			cmd := m.treeView.UpdateAnimation(msg)
			return m, cmd
		}
		return m, nil
...
func (m *Model) checkLoadingComplete() {
	...
	if m.tenant != nil && m.rolesLoaded && m.groupsLoaded && m.lighthouseLoaded {
		...
		if m.client != nil {
			expiry := m.client.TokenExpiry()
			if !expiry.IsZero() {
				remaining := time.Until(expiry)
				m.log(LogDebug, "Token valid for %s (expires %s)", remaining.Round(time.Minute), expiry.Format("15:04"))
			}
		}
	}
}
...
func (m Model) handleKeyPress(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case StateHelp:
		...
		maxScroll := maxHelpLines - maxVisibleLines
		if maxScroll < 0 {
			maxScroll = 0
		}
	...
	case StateTreeView:
		// Handle animation toggle with 'a' key
		if msg.String() == "a" {
			if m.treeView.IsAnimating() {
				m.treeView.StopAnimation()
				m.log(LogInfo, "Stopped flow animation")
				return m, nil
			}
			cmd := m.treeView.StartAnimation()
			m.log(LogInfo, "Started flow animation")
			return m, cmd
		}

		// Handle Esc - stop animation if running, otherwise exit tree view
		if msg.String() == "esc" {
			if m.treeView.IsAnimating() {
				m.treeView.StopAnimation()
				m.log(LogInfo, "Stopped flow animation")
			...
			m.state = StateNormal
			m.log(LogInfo, "Closed tree view")
			return m, nil
		}

		// During animation, ignore navigation keys (only a/esc work)
		if m.treeView.IsAnimating() {
			return m, nil
		}

		// Normal tree navigation - delegate to tree view
		var cmd tea.Cmd
	...
	case "t":
		// Open tree view - shows activated roles flow to tenant
		m.state = StateTreeView
		m.treeView = NewTreeView(m.width-4, m.height-10)
		m.treeView.SetData(m.groups, m.roles, m.tenant)
		m.log(LogInfo, "Opened active roles flow view")
		// Auto-start animation if there are active roles
		if len(m.treeView.Nodes()) > 1 {
			return m, m.treeView.StartAnimation()
		}
		return m, nil
...
func (m *Model) startActivation() (tea.Model, tea.Cmd) {
	...
	return m, func() tea.Msg {
		...
		for _, item := range pending {
			switch v := item.(type) {
			case azure.Role:
				if err := client.ActivateRole(ctx, v.RoleDefinitionID, justification, duration); err != nil {
...
func (m *Model) startDeactivation() (tea.Model, tea.Cmd) {
	...
	return m, func() tea.Msg {
		...
		for _, item := range pending {
			switch v := item.(type) {
			case azure.Role:
				if err := client.DeactivateRole(ctx, v.RoleDefinitionID); err != nil {
