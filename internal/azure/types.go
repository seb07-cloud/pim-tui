...
// TierMetadata represents security tier information for a role
//
// Tier levels:
//   - Tier 0: Control Plane (highest privilege - can escalate to Global Admin)
//   - Tier 1: Data/Workload Plane access
//   - Tier 2: Service Support access
//   - Tier 3: (Azure only) User access
//
// Data sourced from: github.com/emiliensocchi/azure-tiering
type TierMetadata struct {
	Tier             string `json:"tier"`             // "0", "1", "2", "3"
	ID               string `json:"id"`               // Role definition ID
	AssetName        string `json:"assetName"`        // Display name
	AssetDefinition  string `json:"assetDefinition"`  // Description
	DocumentationURI string `json:"documentationUri"` // MS Learn URL
	PathType         string `json:"pathType"`         // "Direct", "Indirect", or empty
	ShortestPath     string `json:"shortestPath"`     // Escalation path description
	Example          string `json:"example"`          // Attack scenario
}
